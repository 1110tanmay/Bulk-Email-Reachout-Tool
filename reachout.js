function sendEmails() {
  ...
  const resumeFileId = "1kza-oXXoKMrxJEFJzOJIQC3kdtcEAMtM"; // ✅ Your Resume File ID
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Personalize the email body with global replacements
    let body = bodyTemplate
      .replace(/{name}/g, data[i][nameIndex] || "")
      .replace(/{lastName}/g, data[i][lastNameIndex] || "")
      .replace(/{position}/g, data[i][positionIndex] || "")
      .replace(/{company}/g, data[i][companyIndex] || "");
